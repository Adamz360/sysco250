...
from .models import Product, Transaction, ProductCategory, ReceiverEmail, LowStockNotification
...
def employee_list(request):
    ...
    return render(request, 'inventory_page/employee_list_page.html', {'employees': employees})


@login_required
def sales_page(request):
    categories = ProductCategory.objects.all()  # Get all categories
    products = Product.objects.all()  # Get all products (can filter per category via JavaScript)
...
def get_product_quantity(request, product_id):
    try:
        product = Product.objects.get(id=product_id)
        return JsonResponse({'available_quantity': product.quantity})  # Assuming `quantity` field exists in Product
    except Product.DoesNotExist:
...
def delete_employee(request, employee_id):
    employee = get_object_or_404(settings.AUTH_USER_MODEL, id=employee_id)
    if request.method == 'POST':
        employee.delete()
...
def notify_low_stock(request, product):
    """
    Notify the user and send an email when stock is low.

    :param request: The HTTP request object, used for notifications.
    :param product: The Product instance being checked.
    """
